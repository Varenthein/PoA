...
<div class="productsAdd" v-loading="loading">
  ...
  <el-breadcrumb separator-class="el-icon-arrow-right">
    ...
    <el-breadcrumb-item :to="{ path: '/users/add' }"><span v-lang.addUser></span></el-breadcrumb-item>
  </el-breadcrumb>

  <!-- TITLE -->
  <h2 class="title" v-lang.addUser></h2>
  ...
  <el-form :model="user" label-position="left" ref="userForm" @submit.native.prevent label-width="120px">
    ...
    <el-row>
      <el-col :span="12">
        ...
        <!-- password -->
        <el-form-item :rules="[{ required: true, message: this.translate('validatePassLengthIsRequired'), trigger: 'blur' }, { min: 6,  message: this.translate('validatePassLength'), trigger: 'blur' }]" :label="translate('password')" prop="password">
          <el-input type="password" v-model="user.password"></el-input>
        </el-form-item>

        <!-- repeat  password -->
        <el-form-item :rules="[{ required: true, message: this.translate('validateRePassIsRequired'), trigger: 'blur' },  { validator: passwordValidator, trigger: 'blur' }]" :label="translate('password_repeated')" prop="password_repeated">
      ...
      <el-col :span="12">
        ...
        <!-- avatar -->
        <el-form-item label="">
          <image-explorer width="300px" height="300px" style="margin-top: 50px;" v-on:chosenImage="user.avatar = $event" />
        </el-form-item>
    ...
    <!-- Add user button -->
    <el-button type="success" size="small" @click="addUser" icon="el-icon-plus">{{ translate('addUser') }}</el-button>
  ...
  methods: {
    ...
    prepareForm() {
        ...
        this.passwordValidator = (rule, value, callback) => {
            if (value === '') {
              callback(new Error(this.translate('validateRePassIsRequired')));
