<template>
<div class="promotionsEdit" v-loading="loading">
  ...
  <el-breadcrumb separator-class="el-icon-arrow-right">
    ...
    <el-breadcrumb-item><span v-lang.editPromotion></span></el-breadcrumb-item>
  </el-breadcrumb>

  <!-- TITLE -->
  <h2 class="title" v-lang.editPromotion></h2>
  ...
  <el-form :model="promotion" label-position="left" ref="promotionForm" @submit.native.prevent label-width="150px">
    ...
    <el-row>
      ...
      <el-col :span="12">
        ...
        <!-- image -->
        <el-form-item label="">
          <image-explorer width="300px" height="300px" style="margin-top: 50px;" v-if="promotion.image !== 0" :thumb="promotion.image.thumb" v-on:chosenImage="promotion.image = $event" />
        </el-form-item>
    ...
    <!-- Add user button -->
    <el-button type="success" size="small" @click="editPromotion" icon="el-icon-plus">{{ translate('editPromotion') }}</el-button>
...
export default {
  ...
  methods: {
    ...
    loadPromotion() {
      ...
      promotionService.getById(this.$route.params.id).then(response => {

        if(response.data && response.data.type) {
            if(response.data.type == "success") {
              this.promotion = response.data.msg
              this.promotion.products_type = 'several'
              this.promotion.amount_less = parseInt(this.promotion.amount_less)
              this.promotion.percent_less = parseInt(this.promotion.percent_less)
              this.promotion.products = JSON.parse(this.promotion.products)
            }
            else this.$message({title: this.translate('error'), message: response.data.msg, type: 'warning'  })
        } else throw 'error'

      }).catch(err => {
        this.$message({title: this.translate('error'), message: this.translate('promotionNotExists')+err, type: 'warning'  })
        setTimeout(() => this.$router.push({ path: '/promotions' }), 3000)
      })
    },
    ...
    editPromotion() {
      ...
      this.$refs['promotionForm'].validate((valid) => {
          ...
          promotionService.editPromotion(promotion.id, promotion).then(response => {
    ...
    addProduct() {
      if(this.promotion.products_type == "single") {
        ...
        if(!(this.promotion.products.find((item) => item.name == this.product.name))) this.promotion.products.push(this.product)
      }
